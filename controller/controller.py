...
class Controller(QObject):
    ...
    def eventHandler_comboBox_period_change(self):
        period = get_keyFromDictValue(self.view.comboBox_period.currentText(), valid_periods)
    ...
    def eventHandler_plainTextEdit_searchTicker_enterPressed(self) -> None:
        """Handles procedure after pressing enter in plainTextEdit_searchTicker"""
        ...
        if self.model.check_duplicates_in_watchlistfile(tickerwrapper):
            self.view.clear_input_field(self.view.plainTextEdit_addTicker)
            return
        self.model.add_tickerinfo_to_watchlistfile(tickerwrapper) # bool_addToWatchlist 1 if symbol shall be added, else 0
    ...
    def eventHandler_plainTextEdit_methodinput(self):
